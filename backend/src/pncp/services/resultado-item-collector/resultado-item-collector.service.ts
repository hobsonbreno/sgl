...
export class ResultadoItemCollectorService {
  ...
  async collectResultadosHomologados() {
    this.logger.log(
      'Iniciando job de coleta OTIMIZADA de resultados homologados...',
    );
        ...
        try {
          ...
          for (const compra of res.data) {
            try {
              ...
              if (
                compra.valorTotalHomologado === null &&
                compra.situacaoCompraId !== 4
              ) {
                continue;
              }
      ...
      this.httpService.get(url, { timeout: 30000 }).pipe(
        retry({
          ...
          delay: (error: AxiosError, retryCount: number) => {
            ...
            this.logger.warn(
              `Falha na requisição para ${url}. Tentativa ${retryCount}/3. Erro: ${error.message}`,
            );
