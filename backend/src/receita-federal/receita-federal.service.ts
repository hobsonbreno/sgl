...
import { EmpresaDataLake, EmpresaDataLakeDocument } from './receita-federal.schema';
...
export class ReceitaFederalService {
  ...
  async runETLPipeline() {
    ...
    for (const i of [0]) { // Apenas 0 para não travar seu teste agora. Mude para `pacotes` depois.
    ...
    for (const i of [0]) { // Apenas 0 para teste.
  ...
  private async loadDicionario(url: string, nome: string): Promise<Map<string, string>> {
    this.logger.log(`Carregando dicionário na memória: ${nome}...`);
    const map = new Map<string, string>();
    return new Promise((resolve, reject) => {
      https.get(url, (res) => {
        if (res.statusCode !== 200) return reject(`Falha no download de ${nome}`);
        const unzip = res.pipe(unzipper.Parse());
        unzip.on('entry', async (entry) => {
          const csvParser = entry.pipe(iconv.decodeStream('win1252')).pipe(parse({ delimiter: ';', relax_quotes: true, quote: '"' }));
  ...
  private async processEstabelecimentos(url: string, targetUf: string, targetStatus: string, mapMunicipios: Map<string, string>, mapCnaes: Map<string, string>) {
          ...
          for await (const row of csvParser) {
            ...
            if (uf === targetUf && situacao === targetStatus) {
              ...
              if (bulkOps.length >= 1000) {
                csvParser.pause();
                try { await this.empresaDataLakeModel.bulkWrite(bulkOps, { ordered: false }); bulkOps = []; } 
                catch (err) {} finally { csvParser.resume(); }
              }
            }
          }
          if (bulkOps.length > 0) await this.empresaDataLakeModel.bulkWrite(bulkOps, { ordered: false });
          entry.autodrain();
        });
        ...
        unzip.on('entry', async (entry) => {
          ...
          for await (const row of csvParser) {
            ...
            if (setCnpjBasicos.has(cnpjBase)) {
              bulkOps.push({
                updateMany: { // Atualiza todas as filiais que tiverem esse cnpj básico
