...
export class ComprasGovScraperService {
  ...
  async scrapeMinhasParticipacoes(): Promise<Map<string, PropostaScrapedData>> {
    ...
    try {
      ...
      if (
        currentUrl.includes('acesso-nao-autorizado') ||
        !currentUrl.includes('sso.acesso.gov.br')
      ) {
        this.logger.log(
          'Acesso bloqueado. Acessando portal de login do Compras.gov.br...',
        );
        ...
        await page.evaluate(() => {
          ...
          ).find((b) => b.textContent?.includes('Entrar com Gov.br'));
      ...
      if (currentUrl.includes('sso.acesso.gov.br')) {
        ...
        await page
          .waitForSelector('#accountId', { timeout: 15000 })
          .catch(() => null);
      ...
      await page
        .waitForSelector('text/PREGÃO', { timeout: 30000 })
        .catch(() => null);
      ...
      const pregoesLength = await page.evaluate(() => {
        ...
        return Array.from(rows).filter((r) => r.textContent?.includes('PREGÃO'))
          .length;
      });

      if (!pregoesLength || pregoesLength === 0) {
        this.logger.warn(
          'Nenhum pregão encontrado ou erro ao carregar a lista de Minhas Participações.',
        );
      ...
      this.logger.log(
        `Encontrados ${pregoesLength} pregões na lista principal.`,
      );
      ...
      for (let p = 0; p < pregoesLength; p++) {
        try {
          ...
          const clicked = await page.evaluate((index) => {
            ...
            const pregoes = Array.from(rows).filter((r) =>
              r.textContent?.includes('PREGÃO'),
            );
            if (pregoes[index]) {
              // Procurar um botão de ação se existir, ou clicar na linha
              const btn = pregoes[index].querySelector(
                'button[icon="pi pi-eye"], i.fa-eye, a',
              ) as HTMLElement;
              if (btn) btn.click();
        ...
        await page.evaluate(() => {
            ...
            const target = items.find((el) =>
              el.textContent
                ?.toLowerCase()
                .includes('itens em que estou participando'),
            ) as HTMLElement;
          ...
          for (let i = 0; i < acompanhamentoLinks.length; i++) {
            ...
            await actPage
              .waitForSelector('text/Todas as propostas', { timeout: 10000 })
              .catch(() => null);
            await new Promise((r) => setTimeout(r, 2000));
            ...
            await actPage.evaluate(() => {
              ...
              const abaTodas = abas.find((el) =>
                el.textContent?.includes('Todas as propostas'),
              ) as HTMLElement;
            ...
            const dadosExtracao = await actPage.evaluate(() => {
              ...
              if (nossaPosicao === 0) {
                const blocosTexto = textoCompleto.split(
                  /(?=\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2})/,
                );
                for (let i = 0; i < blocosTexto.length; i++) {
                  if (
                    blocosTexto[i].includes('GRUPO IRMAOS NASCIMENTO') ||
                    blocosTexto[i].includes('48262939000150')
                  ) {
              ...
              const totalNaFrente = nossaPosicao > 1 ? nossaPosicao - 1 : 0;
              ...
              const propostas = Array.from(
                document.querySelectorAll(
                  'app-proposta, .p-accordion-tab, p-accordiontab, .proposta-row, div.card, div[class*="proposta"]',
                ),
              );
              ...
              if (propostas.length > 0) {
                for (
                  let index = 0;
                  index < Math.min(totalNaFrente, propostas.length);
                  index++
                ) {
                  const text = propostas[index].textContent || '';
                  if (
                    text.toLowerCase().includes('desclassificada') ||
                    text.toLowerCase().includes('recusada')
                  ) {
                    // Adiciona uma nota genérica se não conseguirmos expandir para ler o motivo exato
                    motivosDesclassificacao.push(
                      `Concorrente ${index + 1} foi desclassificado/recusado.`,
                    );
                  }
                }
              } else {
                ...
                const qtdDesclassificados = (
                  textoAteNos.match(/desclassificad[ao]|recusad[ao]/gi) || []
                ).length;
                if (qtdDesclassificados > 0) {
                  motivosDesclassificacao.push(
                    `${qtdDesclassificados} concorrente(s) na frente desclassificado(s).`,
                  );
                }
              }
              ...
              const cabecalho =
                document.querySelector(
                  'app-cabecalho-acompanhamento-compra-fornecedor, .cabecalho, h1, h2, h3',
                )?.textContent || textoCompleto.substring(0, 500);

              return {
                nossaPosicao,
                totalEmpresasNaFrente: totalNaFrente,
                concorrentesDesclassificados: motivosDesclassificacao,
                cabecalhoRaw: cabecalho,
              };
            });

            const itemId =
              actPage.url().split('compra=')[1] || `item-${p}-${i}`;
            ...
            resultados.set(itemId, {
              ...
              concorrentesDesclassificados:
                dadosExtracao.concorrentesDesclassificados,
              uasg:
                dadosExtracao.cabecalhoRaw.match(/UASG:?\s*(\d+)/i)?.[1] ||
                '160045',
              pregao:
                dadosExtracao.cabecalhoRaw.match(
                  /Pregão.*:\s*([\d/]+)/i,
                )?.[1] || '90005/2025',
              qtde: 'N/A',
              valorOfertado: 'N/A',
          ...
          this.logger.log(
            `Finalizado pregão ${p + 1}/${pregoesLength}. Retornando para lista de pregões...`,
          );
          await page
            .goto(
              'https://cnetmobile.estaleiro.serpro.gov.br/comprasnet-web/seguro/fornecedor/compras',
      ...
      this.logger.log(
        `Varredura concluída. Foram processadas ${resultados.size} propostas ativas no total.`,
      );
    } catch (error) {
      this.logger.error(
        'Erro no scraping de Minhas Participações: ' + (error as Error).message,
      );
    } finally {
      if (browser) {
        await browser.close();
