...
export class ComprasnetPublicService {
  ...
  async scrapeSalaDisputa(uasg: string, pregaoNum: string): Promise<any> {
    ...
    try {
      ...
      this.logger.log(`Página carregada, aguardando renderização do Angular SPA...`);
      // Wait for the app to render the main content
      await new Promise(r => setTimeout(r, 12000));
      
      await page.screenshot({ path: `/app/screenshot_${pId}.png`, fullPage: true });
      this.logger.log(`Screenshot salva em /app/screenshot_${pId}.png`);
      
      const data = await page.evaluate(() => {
        // Extrai o chat
        const msgs: string[] = [];
        const chatRows = document.querySelectorAll('app-chat-mensagem, .mensagem-texto');
        ...
        const posRows = document.querySelectorAll('app-classificacao-item, .classificacao-row');
        posRows.forEach(row => {
           positions.push((row as HTMLElement).innerText || row.textContent || '');
        });
      ...
      this.logger.log(`Scrape finalizado para ${pId}. Encontradas ${data.chat.length} msgs e ${data.posicoes.length} itens no ranking.`);
      return data;
      
    } catch (error: any) {
      this.logger.error(`Erro ao fazer scrape da sala de disputa ${pId}: ${error.message}`, error.stack || error);
      throw error;
    } finally {
      if (browser) {
