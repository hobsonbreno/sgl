...
export default function Financeiro() {
  ...
  const carregarDados = async () => {
    try {
      const [resTrans, resResumo, resNegocios, resArquivados] = await Promise.all([
        fetch('http://192.168.1.16:30000/financeiro'),
        fetch('http://192.168.1.16:30000/financeiro/resumo'),
        fetch('http://192.168.1.16:30000/financeiro/negocios-fechados'),
        fetch('http://192.168.1.16:30000/financeiro/arquivados')
      ]);
  ...
  const darBaixaNegocio = async (id: string) => {
    ...
    try {
      await fetch(`http://192.168.1.16:30000/financeiro/receber-negocio/${id}`, { method: 'POST' });
      carregarDados();
    } catch (e) {
      console.error(e);
  ...
  const estornarNegocio = async (id: string) => {
    ...
    try {
      await fetch(`http://192.168.1.16:30000/financeiro/estornar-negocio/${id}`, { method: 'POST' });
      carregarDados();
    } catch (e) {
      console.error(e);
  ...
  const handleSalvar = async (e: any) => {
    e.preventDefault();
    try {
      await fetch('http://192.168.1.16:30000/financeiro', {
        method: 'POST',
  ...
  const marcarPago = async (id: string) => {
    try {
      await fetch(`http://192.168.1.16:30000/financeiro/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
